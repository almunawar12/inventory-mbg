<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4">
    @include('livewire.reports.partials.filter-bar', [
        'title' => 'Customer Nominal',
        'subtitle' => 'Total nominal & kontribusi tiap customer.',
    ])
    ...
    <div class="grid gap-4 md:grid-cols-2">
        <div class="rounded-xl border bg-green-200 bg-card text-card-foreground shadow-sm p-4">
            ...
            <div class="text-xl sm:text-2xl font-bold mt-1">@money($grandTotal)</div>
        </div>
        <div class="rounded-xl border bg-yellow-200 bg-card text-card-foreground shadow-sm p-4">
            ...
            <div class="text-xl sm:text-2xl font-bold mt-1">{{ number_format($invoiceCount) }}</div>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-muted/50 text-left">
                    <tr>
                        ...
                        <th class="px-4 py-2 font-semibold text-right">Total Nominal</th>
                        <th class="px-4 py-2 font-semibold text-right">Kontribusi</th>
                        <th class="px-4 py-2 font-semibold w-1/4">Share</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($rows as $i => $row)
                        <tr class="border-t hover:bg-muted/30">
                            ...
                            <td class="px-4 py-2 text-right font-semibold">@money($row->total_spent)</td>
                            <td class="px-4 py-2 text-right">{{ number_format($row->percentage, 2) }}%</td>
                            <td class="px-4 py-2">
                                <div class="w-full bg-muted rounded-full h-2 overflow-hidden">
                                    <div class="bg-emerald-600 h-2" style="width: {{ min(100, $row->percentage) }}%"></div>
                                </div>
                            </td>
                        </tr>
                    @empty
